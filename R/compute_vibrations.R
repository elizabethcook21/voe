...
vibrate <- function(merged_data,variables_to_vibrate,max_vars_in_model,feature,primary_variable,model_type,regression_weights,max_vibration_num,dataset_id,proportion_cutoff){
    ...
    if(model_type=='negative_binomial'){
        ...
        dataset_id = dataset_id,
        vars = varset,
        full_fits = purrr::map(.data$vars, function(y) tryCatch(broom::tidy(stats::glm(formula=stats::as.formula(paste("I(`",feature,"`) ~ ",primary_variable,'+',paste(y,collapse='+',sep='+'),sep='',collapse='')),weights=regression_df %>% dplyr::select(regression_weights) %>% unlist %>% unname,family=model_type,data = regression_df)),warning = function(w) w, error = function(e) e)),
        feature_fit = purrr::map(.data$full_fits, function(x) tryCatch(dplyr::filter(x, grepl(primary_variable,.data$term)),warning = function(w) w,error = function(e) e)
        )
      ))
    }
...
dataset_vibration <-function(subframe,primary_variable,model_type,features_of_interest,max_vibration_num, proportion_cutoff,regression_weights,cores,logger,max_vars_in_model){
  ...
  if(length(overlap)>0){
    dep_sub = dep_sub %>% dplyr::select(-c(dplyr::all_of(overlap)))
  }
  ...
  if(as.integer(cores)>1){
    ...
    output = furrr::future_map(features_of_interest, function(x) vibrate(merged_data, variables_to_vibrate, max_vars_in_model, x, primary_variable,model_type,regression_weights,max_vibration_num, subframe[[3]], proportion_cutoff))
    dplyr::bind_rows(output)
  }
  else{
    output = purrr::map(features_of_interest, function(x) vibrate(merged_data, variables_to_vibrate, max_vars_in_model, x, primary_variable,model_type,regression_weights,max_vibration_num, subframe[[3]], proportion_cutoff))
    dplyr::bind_rows(output)
  }
}
