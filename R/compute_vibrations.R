...
dataset_vibration <-function(subframe,primary_variable,model_type,features_of_interest,max_vibration_num, proportion_cutoff,cores,logger){#,mtry,num.trees,importance,min.node.size,splitrule){
  ...
  purrr::reduce( # map over all feature's want to vibrate for
   ...
   # purrr::future_map(features_of_interest, function(x) vibrate(in_sub, x, dep_sub,primary_variable,model_type,max_vibration_num, subframe[[3]], proportion_cutoff,logger)),#,mtry,num.trees,importance,min.node.size,splitrule)),
...
compute_vibrations <- function(bound_data,primary_variable,model_type,features_of_interest,max_vibration_num,proportion_cutoff,cores,logger){#,mtry,num.trees,importance,min.node.size,splitrule){
  output = dplyr::bind_rows(apply(bound_data, 1, function(subframe) dataset_vibration(subframe, primary_variable,model_type ,features_of_interest,max_vibration_num, proportion_cutoff,cores,logger)))
