...
get_fudge_factor <- function(metaphlan_df) {
  ...
  return(min(purrr::map_dbl(purrr::map(datatype_df, ~.[. > 0]), ~min(.))))
}
...
dataset_vibration <-function(subframe, features_of_interest,max_vibration_num){
  ...
  reduce( # map over all feature's want to vibrate for
    purrr::map(features_of_interest, function(x) vibrate(subframe[[2]], x, subframe[[1]],primary_variable,max_vibration_num, subframe[[3]])),
...
compute_vibrations <- function(bound_data, primary_variable, features_of_interest,max_vibration_num) {
  output = dplyr::bind_rows(apply(bound_data, 1, function(subframe) dataset_vibration(subframe, features_of_interest,max_vibration_num)))
