...
benchmarkdata = read.csv('jobtimedata',header=F,sep='\t')
benchmarkdata = read.csv('jobtimedata',header=F,sep='\t')
...
benchmarkdata = benchmarkdata %>% select(V1,V4,jobsize,elapsed_time) %>% filter(V1 != '500000')
colnames(benchmarkdata)[2]='Independent variable'
colnames(benchmarkdata)[1]='Number of vibrations'

ggplot(data=benchmarkdata,aes(x=log(`Number of vibrations`),y=log(elapsed_time),color=`Independent variable`)) + geom_point() + ggtitle('Runtime benchmarking') + xlab('log(Number of vibrations)') + ylab('Log(Seconds)')
ggsave('figures/jobtimes.pdf',width=7,height=5)
ggplot(data=benchmarkdata,aes(x=log(`Number of vibrations`),y=log(jobsize),color=`Independent variable`)) + geom_point()+ ggtitle('Memory usage')+ xlab('log(Number of vibrations)') + ylab('Log(RAM used, GB)')
...
phenotypes = unique(benchmarkdata[,2])
...
for(name in ifs){
  ...
  conf_analysis[[name]]=get_confounders(vibration_data,dictionary,name)
  for(i in indvars){
