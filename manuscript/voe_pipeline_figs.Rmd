...
benchmarkdata = benchmarkdata %>% select(V1,V5,V6,elapsed_time,jobsize)
colnames(benchmarkdata)[3]='Independent variable'
colnames(benchmarkdata)[2]='Cores'
colnames(benchmarkdata)[1]='Number of vibrations'

ggplot(data=benchmarkdata %>% filter(Cores==20),aes(x=log(`Number of vibrations`),y=log(elapsed_time),color=`Independent variable`)) + geom_point() + ggtitle('Runtime benchmarking') + xlab('log(Number of vibrations)') + ylab('Log(Seconds)')
ggsave('figures/jobtimes_multicore.pdf',width=7,height=5)

ggplot(data=benchmarkdata %>% filter(Cores==1),aes(x=log(`Number of vibrations`),y=log(elapsed_time),color=`Independent variable`)) + geom_point() + ggtitle('Runtime benchmarking') + xlab('log(Number of vibrations)') + ylab('Log(Seconds)')
ggsave('figures/jobtimes_singlecore.pdf',width=7,height=5)

ggplot(data=benchmarkdata %>% filter(Cores==20),aes(x=log(`Number of vibrations`),y=log(jobsize),color=`Independent variable`)) + geom_point()+ ggtitle('Memory usage')+ xlab('log(Number of vibrations)') + ylab('Log(RAM used, GB)')
ggsave('figures/memusage.pdf',width=7,height=5)

ggplot(data=benchmarkdata %>% filter(Cores==1),aes(x=log(`Number of vibrations`),y=log(jobsize),color=`Independent variable`)) + geom_point()+ ggtitle('Memory usage')+ xlab('log(Number of vibrations)') + ylab('Log(RAM used, GB)')
ggsave('figures/memusage_singlecore.pdf',width=7,height=5)
...
phenotypes = unique(benchmarkdata[,3])
...
for(p in phenotypes){
  ...
  confounder_analysis_list = confounder_analysis_list[, c(1,5,2,6,3,7,4,8)]
  ...
  pheatmap(confounder_analysis_list,cluster_rows=FALSE,cluster_cols=FALSE,main = p,breaks=breaksList)
  dev.off()
}
...
get_confounders <- function(vibration_data,dictionary,name,indfeat){
  ...
  vibration_data_for_reg = vibration_data %>% select(-c(std.error,p.value,statistic,independent_feature)) %>% mutate(estimate=(estimate))
...
# coefficients = as.data.frame(as.matrix(coef(regout))) %>% rownames_to_column('term') %>% arrange(1)
  coefficients = left_join(regout,dictionary %>% select(varname,variable_description),by=c('term'='varname')) %>% filter(!is.na(variable_description)) %>% select(variable_description,term,estimate,std.error)  %>% distinct()
...
for(f in files){
    data=readRDS(f)
    vibration_output=data$vibration_output
    summary_data[[f]] = vibration_output$summarized_vibration_output %>% mutate(dep_feature = strsplit(f,'.rds') %>% map_chr(1) %>% strsplit('10000_') %>% map_chr(2))
    all_vibration_data[[f]]=vibration_output
}

summary_data = bind_rows(summary_data)
write.csv(summary_data,'~/GitHub/voe/manuscript/figures/voe_summary_data.csv')
...
inds_of_interest = c('physical_activity','bDIETARY_FIBER_gm','physical_activity','DR1TTFAT')

for(val in seq(length(deps_of_interest))){
  conf_analysis_sub_asr  = conf_analysis %>% filter(dep_feature == deps_of_interest[[val]], ind_feature == inds_of_interest[[val]]) %>% arrange(desc(estimate)) %>% filter(term == 'RIDRETH1'|term == 'male'| term=='RIDAGEYR'|term=='SES_LEVEL'|term=='education')
ggplot(conf_analysis_sub_asr,aes(x=reorder(variable_description,desc(estimate)),y=estimate)) +geom_bar(stat='identity',position='dodge') + xlab('') + ylab('Change in model effect size')+geom_errorbar(aes(ymin =estimate-std.error,ymax=estimate+std.error), width = 0.2, position = position_dodge(width = 0.9))+ theme(legend.position = 'bottom',plot.title = element_text(size=12))+theme(axis.text = element_text(size=10,angle=30,hjust=1))+ggtitle(paste(deps_of_interest[[val]],'~',inds_of_interest[[val]]))
  ggsave(paste('figures/voeplots/',deps_of_interest[[val]],'_estabilished_confounders','.pdf',sep=''),width=2.5,height=5)
    conf_analysis_sub  = conf_analysis %>% filter(dep_feature == deps_of_interest[[val]], ind_feature == inds_of_interest[[val]]) %>% arrange(desc(estimate))  %>% filter(term != 'RIDRETH1'&term != 'male'& term!='RIDAGEYR'&term!='SES_LEVEL'&term!='education') %>% head(10)
ggplot(conf_analysis_sub,aes(x=reorder(variable_description,desc(estimate)),y=estimate)) +geom_bar(stat='identity',position='dodge') + xlab('') + ylab('Change in model effect size')+geom_errorbar(aes(ymin =estimate-std.error,ymax=estimate+std.error), width = 0.2, position = position_dodge(width = 0.9))+ theme(legend.position = 'bottom',plot.title = element_text(size=12))+theme(axis.text = element_text(size=10,angle=30,hjust=1))
  ...
  for(v in vars){
    full_output_sub = full_output %>% filter(term == inds_of_interest[[val]], independent_feature == deps_of_interest[[val]]) %>% select(estimate,p.value,term,independent_feature,v)
  max_x_val = max(abs(full_output_sub %>% select(estimate) %>% range))+.001
      p1 = ggplot(data = full_output_sub %>% filter(get(v)==1), aes(x = estimate, y = -log10(p.value))) + geom_hex(bins=100)+theme(legend.position="none") +theme(legend.title = element_blank()) + xlab('Estimate') +xlim(-1*max_x_val,max_x_val) + theme(legend.text=element_text(size=6))+ggtitle('')+ theme(plot.title = element_text(size=12)) + geom_hline(yintercept = -log10(0.05)) +scale_color_manual(values=c('blue','gray'))+ylim(0,25)+ xlim(-1*max_x_val,max_x_val) 
    (p1 + geom_hex(data = full_output_sub %>% filter(get(v)==0), aes(x = estimate, y = -log10(p.value)),bins=100,alpha=.1)+theme(legend.position="none") +theme(legend.title = element_blank()) + xlab('Estimate') +xlim(-1*max_x_val,max_x_val) + theme(legend.text=element_text(size=6))+ggtitle('')+ theme(plot.title = element_text(size=12)) + geom_hline(yintercept = -log10(0.05)) +scale_color_manual(values=c('blue','gray'))+ylim(0,25)+scale_fill_gradientn(limits = c(0,1000),breaks=c(250,500,750),colors=c("navyblue", "darkmagenta", "darkorange1")))+ xlim(-1*max_x_val,max_x_val)+ggtitle(paste('Adjusted for ',v,sep='')) 
...
for(name in unique(full_output$independent_feature)){
#  vibration_data_sub1 = full_output %>% filter(independent_feature==name) %>% select(colnames(vibration_data[,topconf]),estimate,p.value)%>% mutate(var=colnames(vibration_data[,topconf]))
...
#    ggplot(data = vibration_data_sub1, aes(x = estimate, color=factor(get(topconf),levels=c(1,0)), y = -log10(p.value))) +geom_point(alpha=.2,shape=21,size=.3,aes(fill=factor(get(topconf),levels=c(1,0))))+theme(legend.position="bottom") +theme(legend.title = element_blank()) + xlab('Estimate') +xlim(-1*max_x_val,max_x_val) + theme(legend.text=element_text(size=6))+ggtitle('')+ theme(plot.title = element_text(size=12)) + geom_hline(yintercept = -log10(0.05)) +scale_color_manual(values=c('blue','grey'))
 #  ggsave(paste('figures/',topconf,'_',name,'_example_top_confounder_analysis.png',sep=''))
  
  vibration_data_sub2 = full_output %>% filter(independent_feature==name)%>% select(colnames(vibration_data[,botconf]),estimate,p.value)%>% mutate(var=colnames(vibration_data[,botconf]))
   max_x_val = max(abs(vibration_data_sub2 %>% select(estimate) %>% range))+.001
   ggplot(data = vibration_data_sub2, aes(x = estimate, color=factor(get(botconf),levels=c(1,0)), y = -log10(p.value))) +geom_point(alpha=alpha,shape=21,size=1.3,aes(fill=factor(get(botconf),levels=c(1,0))))+theme(legend.position="bottom") +theme(legend.title = element_blank()) + xlab('Estimate') +xlim(-1*max_x_val,max_x_val) + theme(legend.text=element_text(size=6))+ggtitle('')+ theme(plot.title = element_text(size=12)) + geom_hline(yintercept = -log10(0.05)) +scale_color_manual(values=c('blue','gray'))+ylim(0,20)
   p1 = ggplot(data = vibration_data_sub2 %>% filter(get(botconf)==1), aes(x = estimate, y = -log10(p.value))) + geom_hex(bins=100)+theme(legend.position="none") +theme(legend.title = element_blank()) + xlab('Estimate') +xlim(-1*max_x_val,max_x_val) + theme(legend.text=element_text(size=6))+ggtitle('')+ theme(plot.title = element_text(size=12)) + geom_hline(yintercept = -log10(0.05)) +scale_color_manual(values=c('blue','gray'))+ylim(0,20)
  p1 + geom_hex(data = vibration_data_sub2 %>% filter(get(botconf)==0), aes(x = estimate, y = -log10(p.value)),bins=100,alpha=.1)+theme(legend.position="none") +theme(legend.title = element_blank()) + xlab('Estimate') +xlim(-1*max_x_val,max_x_val) + theme(legend.text=element_text(size=6))+ggtitle('')+ theme(plot.title = element_text(size=12)) + geom_hline(yintercept = -log10(0.05)) +scale_color_manual(values=c('blue','gray'))+ylim(0,20)+scale_fill_gradientn(limits = c(0,1000),breaks=c(250,500,750),colors=c("navyblue", "darkmagenta", "darkorange1"))
  ggsave(paste('figures/',botconf,'_',name,'_example_bot_confounder_analysis.pdf',sep=''),width=5,height=5)
  vibration_data_sub3=full_output %>% filter(independent_feature==name)%>% select(colnames(vibration_data[,botconf]),estimate,p.value,term)%>% mutate(var=colnames(vibration_data[,botconf]))
  for(f2 in unique(vibration_data_sub3$term)){
   max_x_val = max(abs(vibration_data_sub2 %>% select(estimate) %>% range))+.001
   p1 = ggplot(data = vibration_data_sub3 %>% filter(get(botconf)==1, term==f2), aes(x = estimate, y = -log10(p.value))) + geom_hex(bins=100)+theme(legend.position="none") +theme(legend.title = element_blank()) + xlab('Estimate') +xlim(-1*max_x_val,max_x_val) + theme(legend.text=element_text(size=6))+ggtitle('')+ theme(plot.title = element_text(size=12)) + geom_hline(yintercept = -log10(0.05)) +scale_color_manual(values=c('blue','gray'))+ylim(0,20)
  p1 + geom_hex(data = vibration_data_sub3 %>% filter(get(botconf)==0, term==f2), aes(x = estimate, y = -log10(p.value)),bins=100,alpha=.1)+theme(legend.position="none") +theme(legend.title = element_blank()) + xlab('Estimate') +xlim(-1*max_x_val,max_x_val) + theme(legend.text=element_text(size=6))+ggtitle('')+ theme(plot.title = element_text(size=12)) + geom_hline(yintercept = -log10(0.05)) +scale_color_manual(values=c('blue','gray'))+ylim(0,20)+scale_fill_gradientn(limits = c(0,1000),breaks=c(250,500,750),colors=c("navyblue", "darkmagenta", "darkorange1"))
  ggsave(paste('figures/',botconf,'_',name,'_',f2,'_example_bot_confounder_analysis.pdf',sep=''),width=3,height=3)
  }
}
