...
for(p in phenotypes){
  ...
  pheatmap(confounder_analysis_list,cluster_rows=FALSE,cluster_cols=FALSE,breaks=breaksList,legend = FALSE)
  dev.off()
}
...
for(name in ifs){
  ...
  conf_analysis[[paste(i,name)]]=conf_analysis_temp
    max_x_val = max(abs(vibration_data2%>% select(estimate) %>% range))+.001
...
inds_of_interest = c('physical_activity','bDIETARY_FIBER_gm','LBXCOT','DR1TTFAT')

val = paste(deps_of_interest,inds_of_interest,sep='_')

conf_analysis %>% mutate(temp=paste(dep_feature,ind_feature,sep='_')) %>% filter(temp %in% val) 
...
conf_analysis_sub  = conf_analysis %>% filter(dep_feature == 'LBXTC', ind_feature == 'LBXCOT') %>% arrange(desc(estimate))%>% head(10) %>% select(term) %>% unlist %>% unname %>% as.character()

conf_analysis_sub2  = conf_analysis %>% filter(dep_feature == 'LBXTC', ind_feature == 'LBXCOT') %>% arrange(desc(estimate))%>% head(10) %>% select(variable_description) %>% unlist %>% unname %>% as.character()


forpre = mainTab %>% select(LBXTC,LBXCOT,conf_analysis_sub,WTMEC2YR)

preout = pre(data=forpre,formula=LBXTC ~ . - WTMEC2YR,weights=forpre$WTMEC2YR)
