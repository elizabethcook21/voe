...
benchmarkdata$elapsed_time = as.numeric(gsub('S','',lubridate::seconds(hms(benchmarkdata$time))))
benchmarkdata$jobsize = as.numeric(gsub('K','',benchmarkdata$memory))/1000000

benchmarkdata = benchmarkdata %>% select(vibration_number,cores,phenotype,elapsed_time,jobsize)
...
conf_analysis = conf_analysis %>% mutate(temp=paste(dep_feature,ind_feature,sep='_')) %>% filter(temp %in% val) 
