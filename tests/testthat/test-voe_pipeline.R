test_that("non-meta-analytic pipeline works", {
	...
	abundance = abundance[,1:100]
  	expect_error(full_voe_pipeline(abundance,metadata,primary_variable='BMI',fdr_method='BY',model_type='gaussian',fdr_cutoff=0.99,max_vibration_num=10,proportion_cutoff = 0.9, meta_analysis=FALSE, cores =1, log=FALSE), regexp = NA)
})
